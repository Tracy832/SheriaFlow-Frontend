...
interface CompanyProfile {
  ...
  demo_runs_left?: number; // <--- Added this line
}
...
const Settings = () => {
  ...
  const [formData, setFormData] = useState<CompanyProfile>({
    ...
    has_access: false,
    subscription_expiry: '',
    demo_runs_left: 0 // <--- Added this line
  });
  
  const [userPhone, setUserPhone] = useState(''); // To pass to BillingTab
  ...
  const fetchData = async () => {
    try {
        // Fetch User (for phone number) and Company data
        const [userRes, companyRes] = await Promise.all([
           api.get('/users/me/'), // Assuming this endpoint exists based on earlier context
           api.get('/companies/')
        ]);
        ...
        if (companyRes.data.length > 0) {
            ...
            // Log to confirm backend is sending the data
            ...
            setFormData({
                ...
                demo_runs_left: company.demo_runs_left // <--- Added this line
            });
        }
    } catch (err) {
  ...
  return (
    <div className="p-8 space-y-8 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      ...
      {/* TAB NAVIGATION */}
      <div className="flex gap-6 border-b border-slate-200 mb-8 mt-6">
        {tabs.map((tab) => (
          <button
            key={tab.id}
          ...
          <BillingTab 
            user={{ 
              ...
              company: {
                 ...
                 demo_runs_left: formData.demo_runs_left // <--- Passed down properly
              } 
            }} 
          />
        )}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className={`border rounded-xl p-4 flex items-center justify-between transition-all ${housingLevy ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200'}`}>
                  ...
                  <button onClick={() => setHousingLevy(!housingLevy)} className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${housingLevy ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                ...
                <div className={`border rounded-xl p-4 flex items-center justify-between transition-all ${shifEnabled ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200'}`}>
                  ...
                  <button onClick={() => setShifEnabled(!shifEnabled)} className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${shifEnabled ? 'bg-emerald-500' : 'bg-slate-300'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setNotifications(!notifications)} className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${notifications ? 'bg-emerald-500' : 'bg-slate-300'}`}>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setDarkMode(!darkMode)} className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${darkMode ? 'bg-slate-900' : 'bg-slate-300'}`}>
